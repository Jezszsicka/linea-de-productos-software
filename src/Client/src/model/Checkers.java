...
public class Checkers extends JPanel {
	...
	private class Board extends JPanel implements ActionListener, MouseListener {
		...
		int selectedRow, selectedCol; // If the current player has selected a
										// piece to
		...
		public void paintComponent(Graphics g) {
			...
			if (gameInProgress) {
				...
				if (selectedRow >= 0) {
					g.setColor(Color.white);
					g.drawRect(2 + selectedCol * 20, 2 + selectedRow * 20, 19,
							19);
					g.drawRect(3 + selectedCol * 20, 3 + selectedRow * 20, 17,
							17);
	...
	private static class CheckersData {
		 ...
		 * on the board. The constants RED and BLACK also represent players in
		 * the game.
		 */

		static final int EMPTY = 0, RED = 1, RED_KING = 2, BLACK = 3,
				BLACK_KING = 4;
		 ...
		 * removed from the board. If a piece moves the last row on the
		...
		CheckersMove[] getLegalMoves(int player) {
			...
			ArrayList<CheckersMove> moves = new ArrayList<CheckersMove>(); // Moves
																			// will
																			// be
																			// stored
																			// in
		...
		CheckersMove[] getLegalJumpsFrom(int player, int row, int col) {
			...
			if (player == RED)
				...
				playerKing = BLACK_KING;
			ArrayList<CheckersMove> moves = new ArrayList<CheckersMove>(); // The
																			// legal
																			// jumps
																			// will
																			// be
