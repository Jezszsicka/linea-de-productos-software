...
public class UserInfoUI extends javax.swing.JFrame {
	...
	private void loadRankings(User user) {
		for (int i = 0; i < user.getRankings().size(); i++) {
			...
			JLabel lblRate = new JLabel(String.valueOf(format.format(ranking.getWonGames()
	...
	private JScrollPane getRankingScroll() {
		if(rankingScroll == null) {
			...
			rankingScroll.setBounds(103, 263, 377, 143);
	...
	private JLabel getLblFlag() {
		if(lblFlag == null) {
			...
			lblFlag.setIcon(new ImageIcon(getClass().getClassLoader().getResource("images/es.png")));
		}
		return lblFlag;
	}
